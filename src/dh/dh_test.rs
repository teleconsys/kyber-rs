...
use crate::dh::{Dh, DhXofContext, AES_NONCE_LENGTH};
...
fn test_hkdf_sha256() {
    ...
    let key = DhXofContext::hkdf::<Sha256, Hmac<Sha256>>(&pre_shared_key, &hkdf_context).unwrap();
...
fn test_aes_gcm_interop() {
    ...
    let ciphertext = DhXofContext::aes_encrypt(&key, &nonce, &data, None).unwrap();
    let decrypted = DhXofContext::aes_decrypt(&key, &nonce, &ciphertext, None).unwrap();
...
fn test_aes_gcm_encrypt() {
    ...
    let ciphertext = DhXofContext::aes_encrypt(&key, &nonce, &data, Some(&hdfk_context)).unwrap();
...
fn test_aead_whole() {
    ...
    let encrypted = DhXofContext::aead_encrypt(&point, &hdfk_context, &nonce, &deal_buff).unwrap();
    assert_eq!(encrypted, known_encrypted);

    let decrypted = DhXofContext::aead_decrypt(&point, &hdfk_context, &nonce, &encrypted).unwrap();
