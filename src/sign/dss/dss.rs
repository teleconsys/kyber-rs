...
use crate::{Group, group::{HashFactory, ScalarCanCheckCanonical, PointCanCheckCanonicalAndSmallOrder}, Random, share::{poly::{PriShare, PubPoly, self}, self}, Point, encoding::Marshaling, sign::{eddsa, schnorr}, Scalar};
...
struct DSS<SUITE: Suite, DKS: DistKeyShare<SUITE::POINT>> {
	suite:        SUITE,
	...
	msg:          Vec<u8>,
	partials:     Vec<Option<PriShare<<SUITE::POINT as Point>::SCALAR>>>,
...
fn new_dss<SUITE: Suite, DKS: DistKeyShare<SUITE::POINT>>(suite: SUITE, secret: <SUITE::POINT as Point>::SCALAR, participants: Vec<SUITE::POINT>,
	long: DKS, random: DKS, msg: Vec<u8>, t: usize) -> Result<DSS<SUITE, DKS>> {
...
impl<SUITE: Suite, DKS: DistKeyShare<SUITE::POINT>> DSS<SUITE, DKS> {
    ...
    fn partial_sig(&mut self) -> Result<PartialSig<SUITE>> {
    ...
    fn process_partial_sig(&mut self, ps: PartialSig<SUITE>) -> Result<()> 
    where
    ...
    SUITE::POINT: PointCanCheckCanonicalAndSmallOrder
{
    ...
    fn enough_partial_sig(&self) -> bool {
    ...
    fn signature(&self) -> Result<Vec<u8>> {
    	...
    	let gamma = poly::recover_secret(self.suite.clone(), &self.partials, self.t, self.participants.len())?;
    	// RandomPublic || gamma
...
fn verify<POINT: Point>(public: &POINT, msg: &[u8], sig: &[u8]) -> Result<()> {
...
fn session_id<SUITE: Suite, DKS: DistKeyShare<SUITE::POINT>>(s: SUITE, a: DKS, b: DKS) -> Result<Vec<u8>> {
	let mut h = s.hash();
