...
use crate::{group::edwards25519::SuiteEd25519, Suite, Point, share::{dkg::{self, Config, new_dist_key_handler}, self}, Random, Group, Scalar, sign::dss::DistKeyShare, examples::enc_test::el_gamal_decrypt};
...
fn test_example_dkg() {
	...
	for node in nodes.iter_mut() {
        let dkg = dkg::pedersen::new_dist_key_generator(suite, node.priv_key.clone(), &pub_keys, n).unwrap();
		node.dkg = dkg;
	}
    ...
    for node in nodes.iter() {
        ...
        println!(
            "qualified shares: {:?}", node.dkg.qualified_shares()
        );
        println!(
            "QUAL: {:?}", node.dkg.qual()
        );
	...
	let m = suite.point().embed(Some(message), &mut suite.random_stream());
	let c = suite.point().add( // rA + M
	...
	let decrypted_point = suite.point().sub( // C - (R - pA)
		&c,
