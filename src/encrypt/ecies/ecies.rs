...
use crate::{Group, group::Hasher, Scalar, util::random::Randstream, Point, encoding::{Marshaling, BinaryUnmarshaler}};
...
pub fn encrypt<GROUP: Group>(group: GROUP, public: GROUP::POINT, message: &[u8], hash: Option<fn() -> Box<dyn Hasher>>) -> Result<Vec<u8>> 
{
    let h = match hash {
        Some(h) => h,
        None => std_hash,
...
pub fn decrypt<GROUP: Group>(group: GROUP, private: <GROUP::POINT as Point>::SCALAR, ctx: &[u8], hash: Option<fn() -> Box<dyn Hasher>>) -> Result<Vec<u8>> {
    let h = match hash {
        Some(h) => h,
        None => std_hash,
    };
...
fn derive_key<POINT: Point>(_hash: fn() -> Box<dyn Hasher>, dh: POINT, len: usize) -> Result<Vec<u8>> {
